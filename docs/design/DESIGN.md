...
통계 지도 `/stats`의 바다는 사용자가 지정한 2.5D 원본 재현을 위해 지도 전용 `map-water/light` 토큰의 밝기 변화를 허용한다. 흰 큐브 지형과 `map-visit-low/middle/high` 초록 군집은 지역별 방문 합계 전용 표현이며 일반 버튼이나 브랜드 배경에 재사용하지 않는다.
