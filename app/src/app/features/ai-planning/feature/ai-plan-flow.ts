...
import { COMPANION, PACE, TRANSPORT, SAMPLE_ITEMS, type Phase, type AiPlanSelection } from '../model/ai-plan';
...
export class AiPlanFlow {
  ...
  addRegion(event?: Event): void { event?.preventDefault(); this.draft.addRegion(); }
  removeRegion(index: number): void { this.draft.removeRegion(index); }
  toggle(id: string): void { this.draft.toggle(id); }
  generate(): void { this.draft.generate(); }
  leave(event: Event): void { event.preventDefault(); this.cancel.emit(); }
  commit(): void { if (!this.saving() && this.selected().size) this.apply.emit(this.draft.selection()); }
}
