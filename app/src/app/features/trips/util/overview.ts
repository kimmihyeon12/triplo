...
export type IssueKind =
  'stay-conflict' | 'stay-out-of-range' | 'night-undecided' | 'fixed-time' | 'unverified-location';
...
function nightLabel(trip: Trip, date: IsoDate): { label: string; state: NightState | 'none' } {
  ...
  if (info.tonight.length === 0) {
    return info.checkOuts.length > 0
      ? {
          label: `${info.checkOuts.map((s) => s.name).join(', ')} 체크아웃 · 숙소 미정`,
          state: 'undecided',
        }
      : { label: '숙소 미정', state: 'undecided' };
  }
  if (info.tonight.length > 1)
    return { label: `${info.tonight.map((s) => s.name).join(' / ')} 중복`, state: 'conflict' };
  const stay = info.tonight[0];
  if (info.consecutive) return { label: `${stay.name} 연박`, state: 'covered' };
  if (info.checkOuts.length > 0)
    return {
      label: `${info.checkOuts.map((s) => s.name).join(', ')} 체크아웃 → ${stay.name} 체크인`,
      state: 'covered',
    };
...
export function buildOverview(trip: Trip): TripOverview {
  ...
  const days: OverviewDay[] = undecidedDates
    ...
    : enumerateDays(trip.startDate!, trip.endDate!).map((date, i) => {
        ...
        return {
          ...
          stopNames: dayStops(trip, date)
            .filter((s) => !s.excluded)
            .map((s) => s.name),
          totals: dayTotals(trip, date),
          regionChangeCount: daySegments(trip, date).filter(
            (s) => s.type === 'leg' && s.regionChange,
          ).length,
  ...
  if (conflictNights > 0)
    issues.push({
      kind: 'stay-conflict',
      label: '숙박 날짜 중복',
      count: conflictNights,
      tab: 'stays',
    });
  const outOfRange = [...stayIssues(trip).values()].filter((v) =>
    v.includes('여행 기간 밖'),
  ).length;
  if (outOfRange > 0)
    issues.push({
      kind: 'stay-out-of-range',
      label: '여행 기간 밖 숙박',
      count: outOfRange,
      tab: 'stays',
    });
  const undecidedNights = coverage.filter((n) => n.state === 'undecided').length;
  if (undecidedNights > 0)
    issues.push({
      kind: 'night-undecided',
      label: '숙소 미정인 밤',
      count: undecidedNights,
      tab: 'stays',
    });
  ...
  if (fixedConflicts > 0)
    issues.push({
      kind: 'fixed-time',
      label: '고정 시각 순서 충돌',
      count: fixedConflicts,
      tab: 'days',
    });
  const unverified =
    trip.stops.filter((s) => s.locationStatus === 'unverified').length +
    trip.stays.filter((s) => s.locationStatus === 'unverified').length;
  if (unverified > 0)
    issues.push({
