...
import { RESERVATION_LABEL, type AccommodationStay, type ReservationState, type Trip } from '../model/trip';
...
@Component({
  ...
  templateUrl: './stay-form-page.html',
  styleUrl: './stay-form-page.css',
})
export class StayFormPage {
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      ...
      const id = this.id();
      this.trip.set(null); this.editing.set(null); this.name.set(''); this.address.set(''); this.regionId.set(''); this.checkIn.set(''); this.checkOut.set(''); this.checkInTime.set(''); this.checkOutTime.set(''); this.reservation.set('unknown'); this.memo.set(''); this.location.set(null); this.placeRef.set(null); this.nameTouched.set(false); this.datesTouched.set(false); this.warningsConfirmed.set(false); this.confirmDelete.set(false);
  ...
  async save(event: Event): Promise<void> {
    ...
    if (ok && !this.destroyRef.destroyed && this.id() === trip.id) void this.router.navigate(this.backLink(), { queryParams: { tab: 'stays' } });
  }
