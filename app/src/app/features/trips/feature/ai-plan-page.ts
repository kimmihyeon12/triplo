...
@Component({
  ...
  template: `<app-ai-plan-flow [saving]="store.saveState() === 'saving'" [saveFailed]="store.saveState() === 'error'"
    (apply)="apply($event)" (cancel)="leave()" />`,
})
export class AiPlanPage {
  ...
  constructor() { this.pageBar.set({ title: 'AI 일정 만들기', back: ['/trips'], action: null }); }
  leave(): void { void this.router.navigate(['/trips']); }
  async apply(selection: AiPlanSelection): Promise<void> {
    ...
    if (await this.store.commit(trip) && !this.destroyRef.destroyed) void this.router.navigate(['/trips', trip.id]);
  }
}
