...
import { applyPeriodChange, periodChangeImpact, regionRemovalImpact, removeRegion } from '../util/period';
...
export class TripFormPage {
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      ...
      const id = this.id();
      this.original.set(null); this.title.set(''); this.startDate.set(''); this.endDate.set(''); this.regions.set([]); this.regionInput.set(''); this.impactConfirmed.set(false);
      if (!id) {
  ...
  async save(event: Event): Promise<void> {
    ...
    if (ok && !this.destroyRef.destroyed && this.id() === routeId) void this.router.navigate(['/trips', next.id]);
  }
}
